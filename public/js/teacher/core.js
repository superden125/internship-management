    ...
    if (res.success) {
      ...
      if (document.getElementById) {
        ...
        if (currentDate - endCoreDate === 0) {
          ...
          content = `${formatDate(endCore)} (Đã quá hạn nhập điểm)`;
        } else {
          const diffDays = Math.round(Math.abs((currentDate - endCoreDate) / oneDay))
          content = `${formatDate(endCore)} (Còn ${diffDays} ngày nữa)`;
        }

        document.getElementById('endCore').innerHTML = content;
      }
